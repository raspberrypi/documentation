...
The following command unzips the zip file (replace 2017-06-21-raspbian-jessie.zip with the appropriate zip filename), and pipes the output directly to the dd command. This in turn copies it to the SD card, as described in the previous section.
```
unzip -p 2017-06-21-raspbian-jessie.zip | sudo dd of=/dev/sdX bs=4096
```
...
- To see the progress of the copy operation, you can run the dd command with the status option.
   ```
    dd bs=4M if=2017-06-21-raspbian-jessie.img of=/dev/sdX status=progress
   ```
...
- If the SD card is bigger than the original image size, `dd` will make a copy of the whole card. We must therefore truncate the new image to the size of the original image. Make sure you replace the input file `if=` argument with the correct device name. `diff` should report that the files are identical.
    ...
    truncate --reference 2017-06-21-raspbian-jessie.img from-sd-card.img
    diff -s from-sd-card.img 2017-06-21-raspbian-jessie.img
    ```
