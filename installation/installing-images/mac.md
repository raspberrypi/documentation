...
- Copy the data to your SD card:
    ...
    where `disk` is your BSD name e.g. `sudo dd bs=1m if=2016-02-09-raspbian-jessie.img of=/dev/rdisk4`
    ...
    - If this command still fails, try using `disk` instead of `rdisk`, for example:
       ...
       ```
       `sudo dd bs=1m if=2016-02-09-raspbian-jessie.img of=/dev/disk4`
       ```
       or
       ```
       `sudo dd bs=1M if=2016-02-09-raspbian-jessie.img of=/dev/disk4`
