(function () {
  "use strict";

  if (typeof docsearch === "undefined") {
    return;
  }

  var SPARKLE_SVG =
    '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
    '<path fill-rule="evenodd" clip-rule="evenodd" d="M9 4.5a.75.75 0 0 1 .721.544l.813 2.846a3.75 3.75 0 0 0 2.576 2.576l2.846.813a.75.75 0 0 1 0 1.442l-2.846.813a3.75 3.75 0 0 0-2.576 2.576l-.813 2.846a.75.75 0 0 1-1.442 0l-.813-2.846a3.75 3.75 0 0 0-2.576-2.576l-2.846-.813a.75.75 0 0 1 0-1.442l2.846-.813A3.75 3.75 0 0 0 7.466 7.89l.813-2.846A.75.75 0 0 1 9 4.5ZM18 1.5a.75.75 0 0 1 .728.568l.258 1.036c.236.94.97 1.674 1.91 1.91l1.036.258a.75.75 0 0 1 0 1.456l-1.036.258c-.94.236-1.674.97-1.91 1.91l-.258 1.036a.75.75 0 0 1-1.456 0l-.258-1.036a2.625 2.625 0 0 0-1.91-1.91l-1.036-.258a.75.75 0 0 1 0-1.456l1.036-.258a2.625 2.625 0 0 0 1.91-1.91l.258-1.036A.75.75 0 0 1 18 1.5ZM16.5 15a.75.75 0 0 1 .712.513l.394 1.183c.15.447.5.799.948.948l1.183.395a.75.75 0 0 1 0 1.422l-1.183.395c-.447.15-.799.5-.948.948l-.395 1.183a.75.75 0 0 1-1.422 0l-.395-1.183a1.5 1.5 0 0 0-.948-.948l-1.183-.395a.75.75 0 0 1 0-1.422l1.183-.395c.447-.15.799-.5.948-.948l.395-1.183A.75.75 0 0 1 16.5 15Z" />' +
    "</svg>";

  // The site themes via prefers-color-scheme, but DocSearch's dark theme keys
  // off html[data-theme="dark"]. Mirror the OS preference onto data-theme so the
  // search modal follows light/dark too. (Nothing else on the site reads it.)
  var darkScheme = window.matchMedia("(prefers-color-scheme: dark)");
  syncDocSearchTheme();
  darkScheme.addEventListener("change", syncDocSearchTheme);

  var lvl0 = document.querySelector("#tab-menu li.selected");
  var lvl1 = document.querySelector("#content h1");
  var searchParameters = { optionalFilters: [] };

  if (lvl0 && lvl0.innerText.trim() === "Pico C SDK") {
    searchParameters.optionalFilters.push("hierarchy.lvl0:" + lvl0.innerText.trim());
  }
  if (lvl1) {
    searchParameters.optionalFilters.push("hierarchy.lvl1:" + lvl1.innerText + "<score=2>");
  }

  docsearch({
    appId: "IHWGNTJ1NP",
    apiKey: "94f8e1d1619229399fe8b33bcf750dfd",
    indexName: "raspberrypi",
    container: "#docsearch",
    searchParameters: searchParameters,
    resultsFooterComponent: askAiFooter
  });

  function syncDocSearchTheme() {
    document.documentElement.setAttribute("data-theme", darkScheme.matches ? "dark" : "light");
  }

  function askAiFooter(props) {
    var query = props.state.query;

    if (!query) {
      return null;
    }

    return preact.h(
      "button",
      {
        type: "button",
        "class": "DocSearch-AskAI",
        onClick: function () {
          if (window.Kapa && typeof window.Kapa.open === "function") {
            window.Kapa.open({ mode: "ai", query: query, submit: true });

            var cancel = document.querySelector(".DocSearch-Cancel");
            if (cancel) {
              cancel.click();
            }
          }
        }
      },
      preact.h("span", {
        "class": "DocSearch-AskAI-Icon",
        "aria-hidden": "true",
        dangerouslySetInnerHTML: { __html: SPARKLE_SVG }
      }),
      preact.h("span", { "class": "DocSearch-AskAI-Text" }, "Ask a question about “" + query + "”")
    );
  }
}());
