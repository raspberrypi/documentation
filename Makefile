...
$(DOXYGEN_XML_DIR) $(DOXYGEN_XML_DIR)/index.xml: | $(ALL_SUBMODULE_CMAKELISTS) $(DOXYGEN_PICO_SDK_BUILD_DIR)
	...
	for sdk in ${sdkList} ; do \
		cmake -S $(PICO_SDK_DIR) -B $(DOXYGEN_PICO_SDK_BUILD_DIR)/PICO_RP$$sdk -D PICO_EXAMPLES_PATH=../$(PICO_EXAMPLES_DIR) -D PICO_PLATFORM=rp$$sdk ; \
	done
	$(MAKE) -C $(DOXYGEN_PICO_SDK_BUILD_DIR)/combined docs
	for sdk in ${sdkList} ; do \
		$(MAKE) -C $(DOXYGEN_PICO_SDK_BUILD_DIR)/PICO_RP$$sdk docs ; \
	done
