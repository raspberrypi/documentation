...
The simplest way of playing audio is to use the OMXPlayer application, which is described in more detail [here](../../../raspbian/applications/omxplayer).
...
If you cannot hear anything, make sure your headphones or speakers are connected correctly. Note that omxplayer doesn't use ALSA and so ignores the [audio configuration](../../../configuration/audio-config) set by `raspi-config` or `amixer`.
