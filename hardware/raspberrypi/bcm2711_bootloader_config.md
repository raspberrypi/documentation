...
``N.B. NET_CONSOLE blocks until the ethernet link is established or a timeout occurs. The timeout value is ``DHCP_TIMEOU`` although DHCP is not attempted unless network boot is requested.`` 
...
There will be a post on the Raspberry Pi [General Discussion forum](https://www.raspberrypi.org/forums/viewforum.php?f=63) when the binaries are availble.
...
# Install the BETA bootloader and replace current configuration settings to enable USB boot.
