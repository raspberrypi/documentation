...
**pieeprom-2020-05-15.bin - BETA**
SELF_UPDATE and bootloader_update are now enabled by default so that rpi-update will work by default. Setting either of these parameters to zero prevents self-updates.

### FREEZE_VERSION
Previously this property was only checked by the rpi-eeprom-update script. However, now that self-update is enabled the bootloader will also check this field. If set, this overrides SELF_UPDATE to stop automatic updates. To disable FREEZE_VERSION you will have to use and SD card boot with recovery.bin.
...
** N.B. This blocks boot will block until the ethernet link is established or a timeout (DHCP_TIMEOUT) ** 
...
One way to view the data is to connect the test Pi4 to another Pi running WireShark and select “udp.srcport == 6665” as a filter.
