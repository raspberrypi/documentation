...
Default: -1 (infinite)  
Version: pieeprom-2020-09-03.bin 
...
### ENABLE_SELF_UPDATE
Enables the bootloader to update itself from a network or USB-MSD boot.

If self update is enabled then the bootloader will look for (pieeprom.upd + pieeprom.sig) and/or (vl805.bin + vl805.sig) on the USB MSD boot partition (or TFTP root). If the update files are different to the current image then the update is applied and system is reset. Otherwise, if the images are identical then boot continues as normal.
...
*  Before self-update can be used (e.g. from USB boot) the bootloader must have already been updated to a version which supports self update. The recommended appraoch would be to use the Raspberry Pi Imager to to update to pieeprom-2020-09-03 then use self-update for subsequent updates.

Default: 1  (0 in earlier releases)
Version: pieeprom-2020-09-03.bin  
...
### uart_2ndstage
If set to 0x1 then enable debug logging to the UART. In newer firmware versions (Raspberry Pi OS 2020-08-20) UART logging is also automatically enabled in start.elf. This is also described on the [Boot options](../..//configuration/config-txt/boot.md) page.
...
* 1 - Confiures the write protect regions to cover the entire EEPROM.
...
### bootloader_update
This option may be used to block self-update without requiring the EEPROM configuration to be updated. This is sometimes useful when updating multiple Pis via network boot because this option can be controlled per Pi (e.g. via a serial number filter in config.txt)
...
Network boot is available requires bootloader version pieeprom-2020-04-16.bin or newer.
