...
The Raspberry Pis 3A+ and 3B+ are able to boot from USB without any changes, but the Raspberry Pis 3A and 3B require the USB boot bit to be set in the OTP (one-time programmable) memory. If you are using a Raspberry Pi 3A+ or 3B+, please go to the next section.
