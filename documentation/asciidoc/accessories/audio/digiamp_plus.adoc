...
NOTE: The DigiAMP{plus} is designed to work with Raspberry Pi 3 and earlier devices. 
If you use it with a Raspberry Pi 4 or later devices, use an externally-powered USB hub to prevent the USB ports from adding extra load to the device. Failure to do so is likely to result in a low power warning.
