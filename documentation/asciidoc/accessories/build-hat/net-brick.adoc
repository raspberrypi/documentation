...
WARNING: If you don't call `brick.Dispose()` your program will not terminate.
...
The brick can take a long time before it initialises. A wait for a sensor to be connected has been implemented.
...
It does also take a `CancellationToken` if you want to implement advance features like warning the user after some time and retrying.
