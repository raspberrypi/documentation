...
`libcamera-detect` is not supplied by default in any Raspberry Pi OS distribution, but can be built by users who have xref:libcamera_apps_post_processing_tflite.adoc[installed TensorFlow Lite]. In this case, please refer to the xref:libcamera_apps_building.adoc[`libcamera-apps` build instructions]. You will need to run `cmake` with `-DENABLE_TFLITE=1`.
...
Please refer to the xref:libcamera_apps_post_processing_tflite.adoc#object_detect_tf-stage[TensorFlow Lite object detector] section for more general information on how to obtain and use this model. But as an example, you might spy secretly on your cats while you are away with:
