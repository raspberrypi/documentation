...
KMD and FKMS mode is used by default on the Raspberry Pi 4B. KMD and FKMS uses the DRM/MESA libraries to provide graphics and 3D acceleration.
...
Legacy graphics mode is used by default on all Raspberry Pi models prior to the Raspberry Pi 4B, and can also be used on the Raspberry Pi 4B if required, by disabling KMD and FKMS mode by commenting out the KMD and FKMS line in `config.txt`. 
