...
NOTE: This `rotate=` setting only rotates the text-mode console; any applications that write directly to DRM (e.g. libcamera or cvlc) won't be rotated, and will instead need to use their own rotation options (if available).
