...
Creating a custom base Device tree file is beyond the scope of this documentation, however, it is simple to add an appropriate device tree entry via `config.txt`. See this  xref:../computers/compute-module/cmio-display.adoc#attaching-the-touch-display-2-lcd-panel[page] for configuration details.
