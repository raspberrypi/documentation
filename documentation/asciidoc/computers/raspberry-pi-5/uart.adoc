...
The UART connector is a three-pin header compatible with the Raspberry Pi Debug Connector https://rptl.io/debug-spec[specification]. The xref:../microcontrollers/debug-probe.adoc[Debug Probe] includes a cable terminating in a three-pin JST-SH connector that can connect to the UART.
