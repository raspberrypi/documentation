...
NOTE: The standard, `bcm2711_defconfig`-based kernel (`kernel8.img`) also runs on Raspberrt Pi 5. For best performance you should use `kernel_2712.img`, but for situations where a 4KB page size is required then `kernel8.img` (`kernel=kernel8.img`) should be used.
