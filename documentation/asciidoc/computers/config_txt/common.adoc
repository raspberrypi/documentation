...
NOTE: This option applies only to Raspberry Pi 4B, Compute Module 4, Compute Module 4S, and Raspberry Pi 400.

By default, when connected to a 4K monitor, certain models select a 30 Hz refresh rate. Use this option to allow selection of 60 Hz refresh rates. Models impacted by this setting only support 4Kp60 output on HDMI0. Enabling this setting increases power consumption and temperature.
...
NOTE: This option applies only to later Raspberry Pi 4B revisions, which include two-phase power delivery, and all revisions of Raspberry Pi 400.
