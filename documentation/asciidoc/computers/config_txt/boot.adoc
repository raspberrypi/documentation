...
In 64-bit mode, the firmware will choose an appropriate kernel (for example, `kernel8.img`), unless there is an explicit `kernel` option defined, in which case that is used instead.

Defaults to 1 on Raspberry Pi 4, 400, and Compute Module 4 and 4S platforms. Defaults to 0 on all other platforms. However, if the name given in an explicit `kernel` option matches one of the known kernels then `arm_64bit` is set accordingly.
...
Flagship models since Raspberry Pi 5, Compute Modules since CM5, and Keyboard models since Raspberry Pi 500 _only_ support the 64-bit kernel. Models that only support a 64-bit kernel ignore this flag.
...
If `auto_initramfs` is set to `1`, the firmware looks for an `initramfs` file to match the kernel. The file must be in the same location as the kernel image, and the name is derived from the name of the kernel by replacing the `kernel` prefix with `initramfs`, and removing any extension such as `.img`. For example, `kernel8.img` requires `initramfs8`. You can make use of `auto_initramfs` with custom kernel names provided the names begin with `kernel` and `initramfs` respectively and everything else matches (except for the absence of the file extension on the initramfs). Otherwise, an explicit xref:config_txt.adoc#initramfs[`initramfs`] statement is required.
...
NOTE: Any user-specified OS file can bypass all prefixes by using an absolute path (with respect to the boot partition) – just start the file path with a `/`, for example, `kernel=/my_common_kernel.img`.
...
USB On-The-Go (often abbreviated to OTG) is a feature that allows supporting USB devices with an appropriate OTG cable to configure themselves as USB hosts. On older Raspberry Pis, a single USB 2.0 controller was used in both USB host and device mode.

A high-performance USB 3.0 controller, attached using PCIe, drives the main USB ports on Raspberry Pi 4B and later, and on Raspberry Pi 400 and later. The legacy USB 2.0 controller is still available on the USB-C power connector for use as a device (`otg_mode=0`, the default). Raspberry Pi Compute Module 4 and earlier do not include the USB 3.0 controller.

`otg_mode=1` requests that a more capable XHCI USB 2.0 controller is used as an alternative host controller on that USB-C connector.
...
Specifies a subdirectory/prefix from which to load overlays, and defaults to `overlays/` (note the trailing `/`). If used in conjunction with <<os_prefix,`os_prefix`>>, the `os_prefix` comes before the `overlay_prefix`; for example, `dtoverlay=disable-bt` attempts to load `+${os_prefix}${overlay_prefix}disable-bt.dtbo+`.

NOTE: Unless `+${os_prefix}${overlay_prefix}README+` exists, overlays are shared with the main OS (that is, `os_prefix` is ignored).
...
If this property is set to `1` then the bootloader will attempt load a ramdisk file called `boot.img` containing the xref:configuration.adoc#boot-folder-contents[boot filesystem]. Subsequent files (for example, `start4.elf`) are read from the ramdisk instead of the original boot file system.
...
* The maximum size for a ramdisk file is 128 MB on Pi 4, and 180 MB on Pi 5.
...
Bit 0 (0x1) indicates that the .elf file is custom firmware. This disables any compatibility checks (for example, is USB MSD boot supported) and resets PCIe before starting the executable.
...
When set to `1`, firmware initialises RP1 UART0 to 115200 bps and doesn't reset RP1 before starting the OS (separately configurable using `pciex4_reset=1`).
...
To set the reboot argument at runtime without editing `config.txt`, you can use `vcmailbox`. For example, to set `boot_arg1` to `42` on the next reboot:
...
To set the reboot order at runtime without editing `config.txt`, you can use `vcmailbox`. For example, to test network boot on the next reboot:
...
[NOTE]
====
On Raspberry Pi OS _Bookworm_, the `RuntimeWatchdogSec` parameter is **not enabled by default** and this setting must be configured first in `/etc/systemd/system.conf` before the firmware kernel watchdog can be used.
...
If the kernel watchdog triggers (specifically, the OS fails to reset the watchdog within the timeout), this property specifies the partition number to boot from after the reset. This allows for automatic failover to a recovery or alternate partition.
...
To disable this check (for example, for bare-metal development), set `os_check=0` in config.txt
...
This option may be set to 0 to block self-update without requiring the EEPROM configuration to be updated. This is sometimes useful when updating multiple Raspberry Pis via network boot because this option can be controlled per Raspberry Pi (for example, via a serial number filter in `config.txt`).

Default: `1`

=== Secure Boot configuration properties

[.whitepaper, title="How to use Raspberry Pi Secure Boot", subtitle="", link=https://pip.raspberrypi.com/documents/RP-003466-WP]
****
This whitepaper describes how to implement secure boot on devices based on Raspberry Pi 4. For an overview of our approach to implementing secure boot implementation, please see the https://pip.raspberrypi.com/documents/RP-004651-WP[Raspberry Pi 4 Boot Security] whitepaper. The secure boot system is intended for use with `buildroot`-based OS images; using it with Raspberry Pi OS is not recommended or supported.
****
...
If this property is set to `1` then `recovery.bin` will write the hash of the public key in the EEPROM image to OTP.  When set, the bootloader rejects EEPROM images signed with different RSA keys or unsigned images.
