...
If `enable_rp1_uart` is set to `1` then firmware initialises RP1 UART0 to 115200 bps and doesn't reset RP1 before starting the OS (`pciex4_reset=1`).
This makes it easier to get UART output on the 40-pin header in early boot-code e.g. bare-metal debug.
