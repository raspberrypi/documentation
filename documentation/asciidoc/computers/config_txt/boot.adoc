...
`kernel` is the alternative filename on the boot partition to use when loading the kernel. The default value on the Raspberry Pi 1, Zero, Zero W and Raspberry Pi Compute Module is `kernel.img`, and on the Raspberry Pi 2, 3, 3+, Zero 2 W and Raspberry Pi Compute Module 3 and 3+ it is `kernel7.img`. On the Raspberry Pi 4, 400 and Raspberry Pi Compute Module 4 it is `kernel7l.img`.
