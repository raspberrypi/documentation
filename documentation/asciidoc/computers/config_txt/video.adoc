...
To control HDMI settings, follow the instructions in xref:configuration.adoc#display-settings[Display settings] in the *Configuration* page.

==== HDMI Pipeline for 4-series devices

To support dual displays and modes up to 4Kp60, Raspberry Pi 4B, Compute Module 4, Compute Module 4S, and Raspberry Pi 400 generate two output pixels per clock cycle.
...
Raspberry Pi 5 and later, Raspberry Pi Compute Module 5 and later, and Raspberry Pi 500 and later, also work at two output pixels per clock cycle. These models have special handling for odd timings and can handle these modes directly.
...
By default, the Raspberry Pi Touch Displays are used when detected on the I2C bus. `ignore_lcd=1` skips this detection phase. This prevents the LCD display from being used.

==== `disable_touchscreen`

Enables and disables the touchscreen.

`disable_touchscreen=1` disables the touchscreen component of the official Raspberry Pi Touch Displays.
