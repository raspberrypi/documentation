...
On Raspberry Pi Zero, 1, 2 and 3 the composite video output will be enabled if no HDMI display is detected at boot, or if `hdmi_ignore_hotplug=1` is set. Set `enable_tvout=0` to disable composite even if HDMI is not active.
