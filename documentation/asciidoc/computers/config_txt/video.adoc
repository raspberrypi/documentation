...
The HDMI settings used to be configured by firmware via settings in `config.txt`; now the settings are configured by KMS via xref:configuration.adoc#configure-hdmi-displays[settings] in `cmdline.txt`.
