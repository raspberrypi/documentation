...
Most overclocking issues show up immediately with a failure to boot. If this occurs, remove any clock frequency overrides from `config.txt` then increasing the core-voltage via the `over_voltage_delta` parameter before re-applying over-clocking parameters.
