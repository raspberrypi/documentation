...
NOTE: The `rpicam-apps` supplied with the operating system will be built without any optional 3rd party libraries (such as OpenCV or TensorFlow Lite), meaning that certain post-processing stages that rely on them will not be enabled. To use these stages, please follow the instructions on xref:camera_software.adoc#build-libcamera-and-rpicam-apps[build `rpicam-apps` for yourself].

==== `negate` stage

The `negate` stage requires no third-party libraries.

On a Raspberry Pi 3 or a Raspberry Pi 4 running a 32-bit OS, it may execute more quickly if xref:camera_software.adoc#build-libcamera-and-rpicam-apps[recompiled] using `-DENABLE_COMPILE_FLAGS_FOR_TARGET=armv8-neon`.
