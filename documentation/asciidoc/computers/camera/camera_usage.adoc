...
Apart from some general information on setting up the camera, most of this documentation is divided into sections on xref:camera_software.adoc#libcamera-and-libcamera-apps[libcamera and the libcamera-apps], and the xref:camera_software.adoc#raspicam-commands[legacy _Raspicam_ apps]. Once you've xref:../accessories/camera.adoc#installing-a-raspberry-pi-camera[installed your camera module], please click on one of these two links.
