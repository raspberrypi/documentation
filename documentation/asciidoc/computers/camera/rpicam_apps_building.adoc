...
`-Denable_opencv=enabled`:: Forces OpenCV-based post-processing stages to link or not link. Requires OpenCV to enable. Defaults to `true` when OpenCV is installed, `false` otherwise.
...
NOTE: To disable any of the flags shown above with `enabled` as a value, replace `enabled` with `disabled`. If you specify `auto` instead, the configuration option will automatically be enabled or disabled depending on the availability of the required library dependencies.
