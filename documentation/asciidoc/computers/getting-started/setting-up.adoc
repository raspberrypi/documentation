...
[.whitepaper, title="Transitioning from Pi 3 to Pi 4", subtitle="", link=https://pip.raspberrypi.com/categories/685-whitepapers-app-notes/documents/RP-003475-WP/Transitioning-from-Pi-3-to-Pi-4.pdf]
****
This whitepaper is for those who wish to move from using a Raspberry Pi 3B+ to the Raspberry Pi 4 device.

From a software perspective, the move from the Raspberry Pi 3B+ to the Raspberry Pi 4B is relatively painless as a Raspberry Pi operating system (OS) image should work on all platforms. If, however, you are using a custom kernel, there are some things that will need to be considered in the move.
