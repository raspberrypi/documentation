...
* *Raspberry Pi OS* (desktop with basic software): SD card with at least 32 GB of storage.
* *Raspberry Pi OS Full* (desktop with basic software and recommended applications): SD card with at least 32 GB of storage.
* *Raspberry Pi OS Lite* (a command-line-only version for headless setups): SD card with at least 8 GB of storage.
...
A headless Raspberry Pi operates without a monitor, keyboard, or mouse.

You control it remotely over a network. For this, you must have:

* A network connection (Ethernet or Wi-Fi).
* Remote access using either SSH or xref:../services/connect.adoc[Raspberry Pi Connect] for the first boot of the device. You can use xref:remote-access.adoc#vnc[VNC] for subsequent boots. Note that VNC is not compatible with the Lite versions of Raspberry Pi OS.

We recommend you set up a headless device using *Raspberry Pi Imager*. This allows you to choose between *Raspberry Pi OS* and *Raspberry Pi OS Lite*, and to configure networking, remote access, and user credentials (username and password).

You can enable VNC and *Raspberry Pi Connect* either from the desktop or by using the interactive `raspi-config` tool. For instructions, see xref:configuration.adoc#remote[Remote access].

==== To set up a headless Raspberry Pi using SSH:

Before you start Step 1, note the following:

* Some wireless adapters and Raspberry Pi models don't support 5 GHz networks, so check the documentation for your wireless module to ensure compatibility with your preferred network. For more information, see <<networking>>.

* Because a headless setup doesn't have a desktop, we recommend choosing *Raspberry Pi OS Lite* as your operating system when preparing the boot media using *Raspberry Pi Imager*.

==== Step 1. Prepare your boot media

The first step is to prepare a bootable disk with the Raspberry Pi operating system installed on it.
...
+
On first boot, the only remote access options are SSH or Raspberry Pi Connect. Ensure that you configure one of these options when customising your operating system with Imager. If you choose not to customise the image using Raspberry Pi Imager, you can instead <<manual-ssh-setup, set up SSH manually>>.

. After installing an OS onto your boot media, insert it into your Raspberry Pi or connect it to your Raspberry Pi using a compatible connector.

==== Step 2 (Optional). Connect to a wired network

Next, set up network connectivity so that the headless device can communicate with your computer.

If you used Imager to set up your SSID and wireless network details in Step 1, you can skip this step and go to Step 3.

If you didn't set up a wireless network connection in Step 1, connect the headless device to a wired network now:
...
Before continuing, check that your power supply meets the voltage and current requirements of your Raspberry Pi model. For more information, see <<power>>.

. Plug the power supply into the USB-C (or micro USB on older models) power port on your Raspberry Pi.
...
If you installed a version of Raspberry Pi OS that includes the desktop (that is, the standard or Full version), you can now enable VNC for subsequent connections. For instructions, see xref:configuration.adoc#remote[Remote access].

[[manual-ssh-setup]]
==== Manual setup for SSH

If you didn't customise the image using Raspberry Pi Imager in Step 1, you can instead set up remote access manually.

On your boot media's `bootfs` partition:
