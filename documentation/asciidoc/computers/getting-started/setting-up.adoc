...
VNC and *Raspberry Pi Connect* can be enabled either from the desktop or by using the interactive `raspi-config` tool. For instructions, see xref:configuration.adoc#remote[Remote access].
...
On first boot, the only remote access options are SSH or Raspberry Pi Connect.

If you didn't customise the image using Raspberry Pi Imager in Step 1, set up remote access manually.
...
For detailed information, see xref:../computers/remote-access.adoc#ssh[Access a remote terminal with SSH].