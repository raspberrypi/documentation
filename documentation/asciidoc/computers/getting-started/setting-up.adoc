...
All Raspberry Pi consumer models since the Raspberry Pi 1 Model A+ feature a microSD slot. Models prior to that feature a full-sized SD slot. This documentation will usually say "SD" to refer to either size.

Your Raspberry Pi automatically boots from the SD slot when the slot contains a card.
