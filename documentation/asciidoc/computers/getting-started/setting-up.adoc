...
The boot media storage that you need depends on how much software is included in the OS version you install.

The following recommendations are enough to get started based on your chosen OS, but we recommend choosing more storage space to allow room to grow:
...
Other operating systems might have different storage requirements, so always check the documentation for the OS you plan to install.
...
[[power]]
=== Power supply
Connecting your Raspberry Pi to power is the last step in setting up your hardware. We recommend using an official Raspberry Pi power supply suitable for the Raspberry Pi model you're powering.

Your Raspberry Pi turns on as soon as you connect it to power. This is true unless you've xref:../computers/raspberry-pi.adoc#raspberry-pi-bootloader-configuration[edited your EEPROM settings] to keep your Raspberry Pi 5 device powered off until you press the power button.
...
NOTE: The recommended voltage requirements shown here are measured at the plug, not the supply. Allow for voltage loss in the cable, especially if your power supply cable is removable.
...
[[networking]]
== Networking
The following table lists the connectivity specifications for Raspberry Pi devices.
...
.Raspberry Pi models with built-in connectivity
[cols="2,1,1",options="header"]
|===
|Model Name |Wireless |Ethernet

|Raspberry Pi 3B+ and newer
|Yes. Dual-band Wi-Fi (2.4/5 GHz)
|Yes. 300 Mb/s for Rasberry Pi 3B+, and Gigabit Ethernet for Raspberry Pi 4B and newer
...
|Raspberry Pi 1B, 1B+, and 2B
|No
|Yes

|All keyboard computers (Raspberry Pi 400, 500, and 500+)
|Yes. Dual-band Wi-Fi (2.4/5 GHz)
|Yes. Gigabit Ethernet

|Raspberry Pi Zero W and Zero 2 W
|Yes, 2.4 GHz only
|No

|Compute Module 5 (CM5) and Compute Module 4 (CM4)
|Optional dual-band Wi-Fi (2.4/5 GHz)
|Yes. Gigabit Ethernet.

|Compute Modules 3+ (CM3+) and older
|No
|Optional
|===
...
Use *Raspberry Pi Imager* to install the OS onto your boot media.

This is a prerequisite for using your Raspberry Pi and is where you configure most of its key settings for the first time, including hostname, localisation, user credentials, Wi-Fi connection, and remote access.
...
After connecting your peripherals, plug the power supply onto the USB-C (or micro USB on older models) power port on your Raspberry Pi.

Ensure that your power supply meets the voltage and current requirements of your Raspberry Pi model. For more information, see <<power>>.
...
A headless Raspberry Pi operates without a monitor, keyboard, or mouse. You control it remotely over a network. For this, you must have:

* A network connection (Wi-Fi or Ethernet). Some wireless adapters and some Raspberry Pi models don't support 5 GHz networks; check the documentation for your wireless module to ensure compatibility with your preferred network. For information about which Raspberry Pi models support wireless networks, see <<networking>>.
* Remote access using either SSH or xref:../services/connect.adoc[Raspberry Pi Connect] for the first boot of the device. After first boot, you can also use xref:remote-access.adoc#vnc[VNC] for remote desktop access.

NOTE: VNC is incompatible with the Lite versions of Raspberry Pi OS.

We recommend that you set up a headless device using *Raspberry Pi Imager*. This allows you to:

* Choose between *Raspberry Pi OS* and *Raspberry Pi OS Lite*.
* Configure networking.
* Configure remote access.
* Set user credentials (username and password).

You can additionally enable VNC and *Raspberry Pi Connect* either from the desktop or by using the interactive `raspi-config` tool. For instructions, see xref:configuration.adoc#remote[Remote access].

==== Step 1. Prepare your boot media

Use *Raspberry Pi Imager* to transform your storage device into bootable media with the Raspberry Pi operating system installed. During this process, you also also configure the networking, remote access, and user credentials (username and password) required for a headless setup.

. Follow the instructions to xref:../computers/getting-started.adoc#install[install an OS onto boot media] using *Raspberry Pi Imager*.
    ** Because a headless setup doesn't require a desktop environment, we recommend choosing *Raspberry Pi OS Lite* as your operating system in Raspberry Pi Imager.
    ** On first boot, only SSH and Raspberry Pi Connect are available for remote access. Ensure that you configure one of these options when customising your operating system with Imager. If you choose not to customise the image using Raspberry Pi Imager, you can instead <<manual-ssh-setup, set up SSH manually>>.

. After installing an OS, connect your boot media to your Raspberry Pi.
    ** For most models, this is typically a microSD card inserted into the slot on the underside of the device.
    ** If you installed the OS on any other storage device, you can connect it to your Raspberry Pi using the appropriate adapter or a compatible connector.

==== Step 2. Connect to power

After preparing your boot media and connecting it to your Raspberry Pi, plug the power supply into the USB-C (or micro USB on older models) power port on your Raspberry Pi. Ensure that your power supply meets the voltage and current requirements of your Raspberry Pi model. For more information, see <<power>>.

When your Raspberry Pi powers on, a status LED lights up.

==== Step 3. Connect to a network

After you've installed an OS onto your boot media, inserted it into your Raspberry Pi, and connected your Raspberry Pi to power, you can set up network connectivity so that it can communicate with your computer. This can be a wireless (Wi-Fi) connection (typically set up during step 1 using Imager) or a wired (Ethernet) connection.

===== Connect to a wireless network

NOTE: Some wireless adapters and Raspberry Pi models don't support 5 GHz networks, so check the documentation for your wireless model to ensure compatibility with your preferred network. For more information, see <<networking>>.

If you used Imager to set up your SSID and wireless network details in Step 1, your Raspberry Pi attempts to connect to the wireless network on first boot.

If you didn't set up a wireless network connection in Step 1, connect the headless Raspberry Pi device to a wired network instead. For instructions, see <<headless-ethernet>>.
...
If you used Imager to set up your SSID and wireless network details in Step 1, you can skip this step and go to Step 4.

. Attach an Ethernet cable into your Raspberry Pi device's Ethernet port until you hear a click. If your Raspberry Pi model doesn't include an Ethernet port, use a USB Ethernet adapter.
...
On first boot, the only remote access options to your headless Raspberry Pi are SSH or Raspberry Pi Connect.

* For information about using SSH from another computer, see xref:../computers/remote-access.adoc#connect-to-an-ssh-server[Connect to an SSH server].
* For information about using Raspberry Pi Connect, see the xref:../services/connect.adoc[Raspberry Pi Connect] documentation.

If you installed a version of Raspberry Pi OS that includes the desktop (*Raspberry Pi OS* or *Raspberry Pi OS Full*), you can now enable VNC for subsequent connections. For instructions, see xref:configuration.adoc#remote[Remote access].
...
* *Encrypted password.* Use https://www.openssl.org[OpenSSL] to generate an encrypted password. From the command line, run `openssl passwd -6` and then, when prompted, enter your password and verify it. This command outputs an encrypted version of the supplied password.
+
NOTE: On macOS, ensure that you use OpenSSL to run the command. The default SSL application on macOS is LibreSSL, which doesn't support the `-6` parameter. To verify whether you're running OpenSSL or LibreSSL, run `openssl version`. You can install OpenSSL with `brew install openssl`. For more information, see the documentation for https://openssl.org/[OpenSSL], https://brew.sh/[brew], and macOS.