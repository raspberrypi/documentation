...
Raspberry Pi Imager is a tool that helps you download and write images on macOS, Windows, and Linux. Imager offers a number of operating system images for Raspberry Pi. Imager also provides customisation options so you can preconfigure credentials and remote access settings.
...
* Download the latest version of from https://www.raspberrypi.com/software/[raspberrypi.com/software/] and run the installer.
...
Imager will ask you to apply OS customisation settings via a popup. We recommend configuring your Raspberry Pi via the OS customisation settings. Click the **Edit Settings** button to open the OS customisation menu.
...
The **hostname** option defines the hostname your Raspberry Pi broadcasts to the network using https://en.wikipedia.org/wiki/Multicast_DNS[mDNS]. When you connect your Raspberry Pi to your network, other devices on the network can communicate with your computer using `<hostname>.local`.
...
You can download official images of Raspberry Pi OS at https://www.raspberrypi.com/software/operating-systems/[raspberrypi.com/software/operating-systems]. Alternative and unofficial operating systems for Raspberry Pi computers are available from some third-party vendors.

Imager supports images in both `.img` and `.zip` formats.
...
Currently, Network Install only runs on Raspberry Pi 4, 400, and 5. If your Raspberry Pi runs an older bootloader, you may need to xref:raspberry-pi.adoc#update-the-bootloader[update the bootloader] to use Network Install.
