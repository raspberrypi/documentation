...
* use `apt` to install pre-configured system packages (preferred)
* use `pip` to install libraries using Python's built-in dependency manager
...
Starting in Raspberry Pi OS _Bookworm_,, packages installed via `pip` _must be installed into a Python virtual environment using `venv`._ A virtual environment is a container where you can safely install third-party modules so they won't interfere with your system Python.
...
Before you work on a project, run the following command to start using the virtual environment:
...
Run the following command to start using the virtual environment:
