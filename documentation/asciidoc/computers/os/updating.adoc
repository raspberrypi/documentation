...
If you want to return to the Lite version of Raspberry Pi OS, you can remove these packages. Alternatively, to have the Lite experience without removing the desktop packages installed, use xref:../computers/configuration.html#raspi-config[raspi-config] to set your Raspberry Pi to boot to the command line.
