...
Note that `a full-upgrade` is preferred to a simple `upgrade`, as it also picks up any dependency changes that may have been made.
...
However, there are occasional changes made in the Raspberry Pi OS image that require manual intervention, such as a newly introduced package. These are not installed with an upgrade, as this command only updates the packages you already have installed.
