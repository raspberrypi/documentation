...
The `zip` command compresses a specific file or folder into a zip file. So, for example, `zip -r file.zip ./*` : Use it with `-r` within a specific folder to compress all files or sub-folders into zip files.
