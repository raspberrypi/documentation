...
NOTE: Until recently the default user on Raspberry Pi OS is `pi` with the password `raspberry`. The default user is now set on first boot using a configuration wizard.
