...
NOTE: Previous versions of Raspberry Pi OS made use of a `wpa_supplicant.conf` file, which could be placed into the boot folder to configure wireless network settings. This functionality isn't available from Raspberry Pi OS _Bookworm_ onwards.
...
At the root of the first partition of your SD card (the filesystem labelled `bootfs`), create a file named `userconf.txt`.
