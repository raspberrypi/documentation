...
Further, you need to configure it such that it boots without a screen.
...
[[configuring-screenless-boot]]
=== Configure screenless boot

To enable smooth booting without HDMI connected screen, you may need to tell the boot-procedure that the HDMI may be hotplugged later. This you acheive by removing a comment sign in '/boot/config.txt'.
...
With that, your Raspberry pi should boot both with and without a HDMI cable connected.
