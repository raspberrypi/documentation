...
`start_cd.elf`:: a cut-down version of the firmware that removes support for hardware blocks such as codecs and 3D as well as debug logging support. It also imposes initial frame buffer limitations.

`start4.elf`, `start4x.elf`, `start4db.elf` and `start4cd.elf` are equivalent firmware files specific to the Raspberry Pi 4-series (Model 4B, Raspberry Pi 400, Compute Module 4 and Compute Module 4S).
...
The xref:../computers/configuration.adoc#cmdline[kernel command line] passed into the kernel at boot.
...
Raspberry Pi OS _Bookworm_ includes an `initramfs` file by default. To enable the initial ramdisk, configure it in xref:config_txt.adoc[`config.txt`] with the xref:config_txt.adoc#auto_initramfs[`auto_initramfs`] keyword.

Raspberry Pi OS _Trixie_ includes the `auto_initramfs` keyword by default.
...
| Early revisions of Raspberry Pi 2 used BCM2836
...
| Raspberry Pi 5-optimized xref:config_txt.adoc#boot-options[64-bit kernel].
|===
...
It includes the name of the base CPU, its memory configuration, and any peripherals (internal and external). It is not used to describe the software, although by listing the hardware modules it can cause driver modules to be loaded.
...
Path strings:: Similar to filesystem paths, for example, `/soc/i2s@7e203000` is the full path to the I2S device in BCM2835 and BCM2836. The standard APIs don't create paths to properties like `/soc/i2s@7e203000/status`: instead, you first find a node, then choose properties of that node.
...
The effect of merging that overlay with a standard Raspberry Pi base Device Tree (for example, `bcm2708-rpi-b-plus.dtb`), provided the overlay is loaded afterwards, would be to enable the I2S interface by changing its status to `okay`. But if you try to compile this overlay using:
...
Byte string properties are arbitrary sequences of bytes, for example, MAC addresses. They accept strings of hexadecimal bytes, with or without colons between the bytes.
...
NOTE: As lookup tables operate on input values and literal assignments ignore them, it's not possible to combine the two; any characters after the closing `}` in the lookup declaration are treated as an error.
...
A few property names, when targeted by a parameter, get special handling. One you may have noticed already – `status` –  will convert a boolean to either `okay` for true or `disabled` for false.
...
`rpi-sdram-size-gbit`:: The physical SDRAM size in gigabits. Unlike `/proc/meminfo`, this value isn't affected by memory carveouts or the `total_mem` parameter. In scripts reporting device capabilities, use this value (if defined) instead of the *Memory size* field in the board revision code, which cannot represent non-power-of-two sizes such as 3 GB.

`rpi-machine-id`:: _(string)_ A 128-bit machine identifier represented as a 32-character hexadecimal string, generated from a hash of the 64-bit serial number and, on Raspberry Pi 4 and Raspberry Pi 5, the Ethernet MAC address provisioned at manufacture. This provides a stable, unique identity suitable for use in automated or provisioning systems.

`rpi-min-boot-ver`:: The minimum bootloader manufacturing version required by the board's hardware, programmed into OTP during production. Before applying an update, `rpi-eeprom-update` extracts the `MFG_VER` field from the EEPROM image and compares it against this value. If the image version is lower than the board's minimum, the update is rejected. The `MFG_VER` field embedded in an EEPROM image can be inspected with `strings pieeprom.bin | grep 'MFG_VER:'`. Raspberry Pi 4 and Raspberry Pi 5 only.
...
`max_current`:: The maximum current in mA that the power supply can supply. The firmware reports the value indicated by the USB-C, USB-PD or PoE interfaces. For bench power supplies (for example, connected to the GPIO header) define `PSU_MAX_CURRENT` in the bootloader configuration to indicate the power supply current capability.
...
`build_timestamp`:: The UTC build time for the EEPROM bootloader, stored as a Unix epoch value. The build timestamp is embedded in the EEPROM binary and can be extracted with `strings pieeprom.bin | grep BUILD_TIMESTAMP`.

`capabilities`:: This bit-field describes the features supported by the current bootloader. This may be used to check whether a feature (for example, USB boot) is supported before enabling it in the bootloader EEPROM config.
...
`update_timestamp`:: The UTC time when the EEPROM update was applied, stored as a Unix epoch value. This value is set by `rpi-eeprom-update` and stored in the `.sig` file alongside the EEPROM image. The bootloader uses this to avoid applying stale updates during self-update.
...
* Power-on – pins default to inputs with default pulls, which are described in the https://pip.raspberrypi.com/documents/RP-008249-DS[BCM2835 datasheet]
...
* Additional firmware pins (for example, UARTS)
* Kernel/Device Tree
...
* `pins_cm0`: Raspberry Pi Compute Module Zero
...
* `pins_4b`: Raspberry Pi 4 Model B
