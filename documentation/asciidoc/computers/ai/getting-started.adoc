...
If you're using an AI Kit, we recommend that you enable PCIe Gen 3.0. You can skip this for AI HAT+ and AI HAT+ 2 because the setting is automatically applied.

By default, Raspberry Pi 5 uses Gen 2.0 (5 GT/s) speeds on its PCIe interface. To achieve the best performance for your NPU, use one of the following approaches to force Gen 3.0 (8 GT/s) speeds:

- Enable Gen 3.0 on your Raspberry Pi 5 from the configuration command line (CLI) tool (*`raspi-config`*).
- Update the configuration file (*`config.txt`*) to force the PCIe interface on your Raspberry Pi 5 to operate at PCIe Gen 3.0 speeds.

NOTE: This step is optional but highly recommended. For more information about this setting, see xref:raspberry-pi.adoc#pcie-gen-3-0[PCIe Gen 3.0].
...
----
.  Reboot your Raspberry Pi to force the settings to take effect:
+
[source,console]
----
$ sudo reboot
...
This section page provides instructions for setting up the Hailo 10 NPU on your AI HAT+ 2 with a Raspberry Pi 5 so that you can locally run Large Language Models (LLM). With the following setup, you can access LLMs through:
...
The *Hailo Gen-AI Model Zoo* contains the pre-trained LLMs suitable for running on Hailo-10H. These models are installed as part of <<step2-llm>>. These models are then loaded by the runtime and run by the AI accelerator.
...
The backend is installed as part of <<step3-llm>>.

==== Frontend layer (optional)

The frontend layer is an optional, browser-based chat interface provided by *Open WebUI*. While Open WebUI isn't required to run LLMs, it provides a more user-friendly way to submit prompts and view responses instead of terminal-based POST requests. The frontend communicates with the backend through its REST API and displays model outputs in a conversational UI.

The frontend is installed as a combination of <<step1-llm>> and <<step4-llm>>.

[[step1-llm]]
=== Step 1. Install Docker (for Open WebUI)

You can skip this step if you don't plan to use the Open WebUI interface.

You must use Docker if you want to use the Open WebUI front end. This is because the Web UI is incompatible with Python 3.13 on Trixie. Docker provides a containerised environment for stable operation. For more comprehensive installation instructions, see Docker's page: https://docs.docker.com/engine/install/debian/[Install Docker Engine on Debian].
...
[[step2-llm]]
=== Step 2. Install Hailo Ollama server

The Hailo Ollama server provides:

- The pre-trained LLMs optimised for the Hailo-10H NPU.
- The Ollama back end that exposes a REST API for querying models.

To install the Hailo Ollama server, follow the instructions in https://github.com/hailo-ai/hailo_model_zoo_genai[Hailo's Model Zoo GitHub repository]. Ensure that you download *version 5.1.1* of the Hailo Model Zoo GenAI package.


[[step3-llm]]
=== Step 3. Start the Hailo Ollama server and run LLMs
...
You can set up *Open WebUI*, an optional browser-based, chat interface that connects to the Hailo Ollama server, allowing you to send queries to, and view responses from, LLMs using a conversational UI. Alternatively, you can keep using the terminal-based POST requests described in <<step3-llm>> to interact with LLMs.

Open WebUI needs to run in a Docker container. You install Docker in <<step1-llm>>. You must now deploy and run an Open WebUI container with Docker.
...
. To use Open WebUI, you first need to install it. Download the Open WebUI image required to run the frontend:
+
[source, console]
----
$ docker pull ghcr.io/open-webui/open-webui:main
----
. Ensure that `hailo-ollama` is already running. Then, start the Open WebUI container and connect it to the `hailo-ollama` backend:
+
[source, console]
----
