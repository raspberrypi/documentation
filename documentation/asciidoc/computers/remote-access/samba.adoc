...
This error occurs when SMB protocol versions don't match and the Linux Samba client returns a misleading error message. By default, Raspberry Pi OS uses versions 2.1 and later, compatible with Windows 7 and later. Older devices, including some NAS devices, might require version 1.0. To fix this error, append a version entry (for example, `,vers=1.0`) to your mount command:
...
You might need to try different versions to match up with the server version. Possible values are:
