...
First, find the IP address of the computer you're using with the following command:

----
hostname -I
----

You should see output similar to the following if your network uses IPv6:

----
fd00::ba27:ebff:feb6:f293 2001:db8:494:9d01:ba27:ebff:feb6:f293
----

The example shows two IP addresses. The first one is a _unique local unicast address_ (`fc00::/7`). The second one is the _global unicast address_ (`2000::/3`). You may see only one of them depending on your network configuration. You can reach your Raspberry Pi within your LAN using either address. The address out of `2000::/3` is accessible world wide, provided your router's firewall is opened.
...
* `-c 2` sends two echo requests

* `-I` sets the interface and the source address of the echo request

* `ff02::1` is a well-known multicast address for all nodes on the link, so it behaves like a local broadcast

TIP: Usually a well-known multicast address is defined in `/etc/hosts`. As a result, you can also use `ip6-allnodes` or `ipv6-allnodes` instead of a literal address. Check your `/etc/hosts` file to be sure.
...
When you successfully connect over SSH as demonstrated in the example above, you have identified the address of your Raspberry Pi.
