...
You can set up a DHCP/TFTP server which allows you to boot a Raspberry Pi 3 or later from the network.
...
Before the Raspberry Pi 3 Model B can network boot it needs to be booted from an SD card with a config option to enable USB boot mode. This sets a bit in the OTP (One Time Programmable) memory in the Raspberry Pi SoC that enables network booting. When this is done, the Raspberry Pi 3B attempts to boot from USB, and from the network, if it can't boot from the SD card.
...
This adds `program_usb_boot_mode=1` to the end of xref:config_txt.adoc#what-is-config-txt[`/boot/firmware/config.txt`]. Reboot the Raspberry Pi with `sudo reboot`. After the client Raspberry Pi has rebooted, check that the OTP has been programmed with:
...
Within `raspi-config`, choose `Advanced Options`, then `Boot Order`, then `Network Boot`. You must then reboot the device for the change to the boot order to be programmed into the bootloader EEPROM. Atfer the Raspberry Pi reboots, check that the boot order is now `0xf21`:
