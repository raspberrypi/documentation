...
The Ethernet jack on Raspberry Pi 4B and 3B+ is PoE capable, supporting the IEEE 802.3af-2003 PoE standard.
...
The following table describes the amount of current (in amps) drawn by different Raspberry Pi models during various workloads:
...
[.whitepaper, title="Extra PMIC features on Raspberry Pi 4, Raspberry Pi 5 and Compute Module 4", subtitle="", link=https://pip.raspberrypi.com/documents/RP-004340-WP]
****
...
By default, the Raspberry Pi 5 consumes around 1 W to 1.4 W of power when turned off. This can be decreased by manually editing the EEPROM configuration with `sudo rpi-eeprom-config -e`. Modify the settings to add:
...
[.whitepaper, title="Making a more resilient file system", subtitle="", link=https://pip.raspberrypi.com/documents/RP-003610-WP]
****
...
reset_event:: The PMIC reset reason, for example, watchdog, over- or under-voltage, over-temperature
