...
NOTE: Early editions of Raspberry Pi 4 might require a xref:../computers/configuration.adoc#update-bootloader-version[bootloader update] to boot from USB.
...
To enable USB host boot mode on these devices, set the USB host bit in OTP (one-time programmable) memory. To set the bit, boot from an SD card where xref:config_txt.adoc#what-is-config-txt[`/boot/firmware/config.txt`] contains the line `program_usb_boot_mode=1`. After you set the bit, you can boot from USB without the SD card.
...
The xref:getting-started.adoc#install[procedure] is the same as for SD cards – flash the USB storage device with the operating system image.
