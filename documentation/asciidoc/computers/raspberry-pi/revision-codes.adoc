...
^3^ For a memory size of "Other", the actual memory capacity can be determined from the `rpi-sdram-size-gbit` xref:configuration.adoc#part4[firmware parameter].
...
To avoid problems when new board revisions are created, do not use the revision code (for example, `c03111`).
...
| Raspberry Pi 2 Model B Rev 2  | `raspberrypi`   | `2-model-b-rev2`            | `brcm`     | `bcm2837`
