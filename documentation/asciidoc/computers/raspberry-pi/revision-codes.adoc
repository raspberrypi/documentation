...
In this example above, we have a hexadecimal revision code of `c03111`, converting this to binary we get `1 100 0000 0011 00010001 0001`. Spaces have been inserted to show to borders between each section of the revision code, according to the above table.

So, starting from the lowest order bits, the bottom four (0-3) are the board revision number, so this board has a revision of 1. The next four bits (4-11) are the board type, in this case binary `10001`, hex `11`, so this is a {pi} 4B. Using the same process, we can determine that the processor is the 2711, the board was manufactured by Sony UK, and it has 4GB of RAM.
...
Raspberry Pi advise against using the revision code as a whole (`c03111`). For example, you might consider having a list of possible revision codes you support in your program, and compare the detected code with your list to determine if you allow your program to run. However, this mechanism will break when a new board revision comes out, or if the production location changes. For example, Raspberry Pi occasionally silently release newer versions of our devices and these new boards get new board revision numbers. This changes the overall revision code and this new revision code will not be in your programs list. Your program will now reject the unrecognised code, and perhaps abort, even though revisions of the same board type are always backwards compatible. You would need to release a new version of software with the specific revision added to the list, which can be a maintenance issue.

Another example might be if your program is only intended to work on devices with 2GB or RAM or more. The naive approach is to look at the list of revision codes for models that have 2GB of RAM or more, and build that list in to your code. But of course, this breaks as soon as a new board revision is released, or the boards are manufactured at a different location.

A better mechanism is to just use the board type field (3A, 4B etc) to determine which model your program supports, or perhaps just the amount of memory field. So you might say you will support any Raspberry Pi 4B's, whatever their board revision code, because that should always work. Or you might want to restrict your code to 4B devices with 2GB of RAM or more. Simply look at those two fields to determine whether you are going to allow you code to run.

In short, the advice is to ignore those fields that are not relevant to your determining whether your code will run, meaning you should not use a list of complete revision codes for identification.
...
One further thing to note, you should always check bit 23, the 'New' flag, to ensure that the revision code is the new version before checking any other fields. The examples also do this.
