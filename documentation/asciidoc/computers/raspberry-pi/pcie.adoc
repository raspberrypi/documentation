...
Raspberry Pi 5 has an FPC connector on the left side of the board. This connector breaks out a PCIe Gen 2.0 ×1 interface for fast peripherals.

To connect a PCIe https://pip.raspberrypi.com/documents/RP-008281-DS[HAT+ device], connect it to your Raspberry Pi. Your Raspberry Pi should automatically detect the device. To connect a non-HAT+ device, connect it to your Raspberry Pi, then <<enable-pcie, manually enable PCIe>>.

For more information about the PCIe FPC connector pinout and other details needed to create third-party devices, accessories, and HATs, see the https://pip.raspberrypi.com/documents/RP-008298-DS[Raspberry Pi Connector for PCIe] standards document. It should be read alongside the https://pip.raspberrypi.com/documents/RP-008281-DS[Raspberry Pi HAT+ Specification].
...
By default, Raspberry Pi 5 uses Gen 2.0 speeds (5 GT/s). Use one of the following approaches to enable Gen 3.0 (8 GT/s) speeds:
