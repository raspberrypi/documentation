...
NOTE: On 2712 these numbers are different. Row 31 is the serial number and row 32 is the board revision number. The customer rows are 77 to 84 inclusive
...
=== Making Customer OTP bits Unreadable (2711 only)
...
=== Customer MAC addresses (2712 only)

On 2712 the Ethernet, Wifi and Bluetooth MAC addresses are set in OTP memory. These values can be change with customer values.

Get customer mac address `vcmailbox 0x00030082/3/4 6 6 0 0` Where 2 is ethernet, 3 is wifi and 4 is BT.
...
To set a customer mac address it has to be sent as two 32 words with the bytes in the right order. You can run a command to check it's formatted properly.
...
Check the log to see if the mac is what's expected...
[,bash]
----
sudo vclog -m
...
NOTE: This is not yet implemented on 2712
