...
[.whitepaper, title="Using the one-time programmable memory on Raspberry Pi single-board computers", subtitle="", link=https://pip.raspberrypi.com/documents/RP-003611-WP]
****
All Raspberry Pi single-board computers (SBCs) have an inbuilt area of one-time programmable (OTP) memory, which is actually part of the main system on a chip (SoC). As its name implies, OTP memory can be written to (that is, a binary 0 can be changed to a 1) only once. When a bit has been changed to 1, it can never be returned to 0. One way of looking at the OTP is to consider each bit as a fuse. Programming it involves deliberately blowing the fuse — an irreversible process, as you cannot get inside the chip to replace it!
...
WARNING: The OTP values are one-time programmable. After a bit has been changed from 0 to 1, it can't be changed back.
...
When locked, the customer OTP values can no longer be altered. Note that this locking operation is irreversible.
