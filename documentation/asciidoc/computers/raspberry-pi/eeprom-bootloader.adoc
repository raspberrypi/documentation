...
If `1` then enable firmware debug output on the primary UART (the UART pointed to by `/dev/serial0`).

On Raspberry Pi 5, this is `UART10` by default, which is exposed on the debug UART connector. However, in some configurations, Raspberry Pi 5 exposes the primary UART on GPIO 14 and 15 (`UART0`). On Raspberry Pi 4, the primary UART is exposed on GPIO 14 and 15. For more information, see xref:configuration.adoc#primary-and-secondary-uart[Primary and secondary UART].

Configure the receiving debug terminal at 115200 bps, 8 data bits, 1 stop bit, no parity.
...
This setting is not relevant for Raspberry Pi 5 and later, Raspberry Pi Compute Module 5 and later, and Raspberry Pi 500 and later, because the xref:raspberry-pi.adoc#power-button[dedicated power button] can always be used to wake from `HALT` or `STANDBY`.
...
If `1` and `WAKE_ON_GPIO=0` then `sudo halt` will switch off all PMIC outputs. This is lowest possible power state for halt but might cause problems with some HATs because 5V will still be on. `GLOBAL_EN` must be shorted to ground to boot.

The dedicated power button on Raspberry Pi 400 operates even if the processor is switched off. This behaviour is enabled by default, however, `WAKE_ON_GPIO=2` can be set to use an external GPIO power button instead of the dedicated power button.

On Raspberry Pi 5 and later, and on Raspberry Pi 500 and later, this places the PMIC in `STANDBY` mode where all outputs are switched off. There is no need to set `WAKE_ON_GPIO` and pressing the xref:raspberry-pi.adoc#power-button[dedicated power button] boots the device.
...
The `BOOT_ORDER` property defines the sequence for the different boot modes. It is read right to left, and up to eight digits can be defined.
...
| 0x6
| NVME
| CM4, CM5, Raspberry Pi 5, and 500+ only: boot from an NVMe SSD connected to the PCIe interface. For more information, see xref:raspberry-pi.adoc#nvme-ssd-boot[NVMe boot].
...
| 0xf
| RESTART
| Restart from the first boot-mode in the `BOOT_ORDER` field (loop).
|===

`RPIBOOT` is normally enabled implicitly by selecting the `nRPIBOOT` jumper on a CM4 or CM5 IO board. However, it can be useful to set this within a custom `BOOT_ORDER` for development or provisioning, for example, by using `set_reboot_order`.
If used, always set it as the last option in the `BOOT_ORDER` because this boot mode doesn't support timeouts or retries.
...
This is useful for unattended or remote systems to ensure recovery from failed boots (for example, if the OS never loads).
...
If the RESTART (`0xf`) boot-mode is encountered more than MAX_RESTARTS times then a watchdog reset is triggered. This isn't recommended for general use but might be useful for test or remote systems where a full reset is needed to resolve issues with hardware or network interfaces.
...
If a fatal error is encountered during boot, the bootloader displays the error pattern on the activity LED three times and then performs a hard reset. This can mitigate intermittent hardware issues caused by power supplies, HATs, or board temperature. This setting only applies to the bootloader and has no effect after the OS has started.

Displaying the error pattern three times before resetting rate-limits reboots. If a faster reboot is required, use `BOOT_WATCHDOG_TIMEOUT` instead.

Set `REBOOT_ON_FATAL_ERROR=0` to disable automatic rebooting and instead display the error pattern indefinitely.
...
The timeout in milliseconds for an individual file download through TFTP.
...
Optional dotted decimal ip address (for example, `192.168.1.99`) for the TFTP server which overrides the server-ip from the DHCP request.

This might be useful on home networks because tftpd-hpa can be used instead of dnsmasq where broadband router is the DHCP server.
...
On earlier models the serial number is used as the prefix, however on Raspberry Pi 4 and 5 the MAC address is no longer generated from the serial number, making it difficult to automatically create tftpboot directories on the server by inspecting DHCPDISCOVER packets. To support this, you can customise the TFTP_PREFIX to either be the MAC address, a fixed value or the serial number (default).

|===
| Value | Description

| 0
| Use the serial number, for example, `9ffefdef/`

| 1
| Use the string specified by `TFTP_PREFIX_STR`

| 2
| Use the MAC address, for example, `dc-a6-32-01-36-c2/`
|===
...
In earlier releases the client GUID (Option97) was just the serial number repeated four times. By default, the new GUID format is the concatenation of the four-character code (FourCC) (`RPi4` `0x34695052` for Raspberry Pi 4  or `RPi5` `0x35695052` for Raspberry Pi 5), the board revision (for example, `0x00c03111` or `0x00d04170`) (4-bytes), the least significant 4 bytes of the mac address and the 4-byte serial number.
...
Overrides the Raspberry Pi Ethernet MAC address with the given value, for example, `dc:a6:32:01:36:c2`
...
Any two customer rows can be selected and combined in either order.
...
If TFTP_IP and the following options are set then DHCP is skipped and the static IP configuration is applied. If the TFTP server is on the same subnet as the client, `GATEWAY` can be omitted.

[[CLIENT_IP]]
===== `CLIENT_IP`

The IP address of the client, for example, `192.168.0.32`
...
The subnet address mask, for example, `255.255.255.0`
...
The gateway address to use if the TFTP server is on a different subnet, for example, `192.168.0.1`
...
* For network boot make sure that the TFTP `boot` directory can be mounted through NFS and that `rpi-eeprom-update` can write to it.
...
The SHA-256 hash (as a hex string) of a DER-encoded CA certificate to use for HTTPS network install and HTTP boot when `HTTP_HOST` is set to a custom server. The CA certificate itself is embedded in the EEPROM image using `rpi-eeprom-config --cacertder`.

When `HTTP_CACERT_HASH` is specified alongside `HTTP_HOST` and `HTTP_PATH`, network install and HTTP boot use HTTPS over port 443 instead of plain HTTP over port 80.

See xref:raspberry-pi.adoc#http-boot[HTTP boot] for more details.
...
In order to detect the keyboard, network install must initialise the USB controller and enumerate devices. This increases boot time by approximately 1 second so it might be advantageous to disable network install in some embedded applications.
...
`NETCONSOLE` should not be enabled by default because it might cause network problems. It can be enabled on demand through a GPIO filter:
...
The `PARTITION` option can be used to specify the boot partition number, if it has not explicitly been set by the `reboot` command (for example, `sudo reboot N`) or by `boot_partition=N` in `autoboot.txt`.
This could be used to boot from a rescue partition if the user presses a button.

The latest firmware also allows high partition numbers (> 31) to be overridden. This allows a custom setup of the system hardware watchdog to trigger a reboot with a special high partition number (for example, 62) which can be detected by the bootloader (using a conditional filter) and remapped to a recovery partition.
...
If set to `1`, the bootloader enumerates the PCIe x1 interface to discover NVMe devices during boot. This is required for custom PCIe expansion designs that don't support the HAT+ specification. HAT+ compliant devices are auto-detected and don't require this setting.
...
Typically, this is either set to `3000` or `5000` for either a low- or high-current capable power supply.
...
This is intended to allow problematic (for example, very slow to enumerate) devices to be ignored during boot enumeration. This is specific to the bootloader and is not passed to the OS.
...
How long to wait in milliseconds before advancing to the next LUN, for example, a multi-slot SD-CARD reader. This is still being tweaked but might help speed up boot if old/slow devices are connected as well as a fast USB-MSD device containing the OS.
...
When the Raspberry Pi is rebooted, USB power is switched off by the hardware. A short power-off time can cause problems with some USB devices so this parameter can be used to force a longer power off, as though the cable was physically removed.

On Raspberry Pi 4 version 1.3 and older, the configurable/long power off requires the XHCI controller to be enabled so there is actually a short power off followed by a longer configurable power off. You can skip the longer configurable power off by setting this parameter to zero.
...
If defined, delays USB enumeration for the given timeout after the USB host controller has initialised. If a USB hard disk drive takes a long time to initialise and triggers USB timeouts then this delay can be used to give the driver additional time to initialise. It might also be necessary to increase the overall USB timeout (`USB_MSD_DISCOVER_TIMEOUT`).
...
This property is a bit-field which controls the verbosity of USB debug messages for mass storage boot-mode. Enabling all of these messages generates a huge amount of log data which will slow down booting and might even cause boot to fail. For verbose logs it's best to use `NETCONSOLE`.
...
SDRAM_BANKLOW controls how the SDRAM banks are arranged within the system address space. The number of bank bits can be selectively mapped to being located in the MSBs of the address, or being located between the row and column address bits.
This setting can significantly affect SDRAM performance.

When unset, the bootloader will choose the preferred option. This is currently 3 on Raspberry Pi 4 family and 1 on Raspberry Pi 5 family.
...
NOTE: On a Raspberry Pi 5 family device, if NUMA is not available in the kernel then performance will be adversely affected.
