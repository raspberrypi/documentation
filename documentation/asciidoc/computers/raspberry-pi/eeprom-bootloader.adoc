...
The dedicated power button on Raspberry Pi 400 operates even if the processor is switched off. This behaviour is enabled by default, however, `WAKE_ON_GPIO=2` may be set to use an external GPIO power button instead of the dedicated power button.
...
The `PARTITION` option may be used to specify the boot partition number, if it has not explicitly been set by the `reboot` command (for example, `sudo reboot N`) or by `boot_partition=N` in `autoboot.txt`.
...
How long to wait in milliseconds before advancing to the next LUN, for example, a multi-slot SD-CARD reader. This is still being tweaked but may help speed up boot if old/slow devices are connected as well as a fast USB-MSD device containing the OS.
