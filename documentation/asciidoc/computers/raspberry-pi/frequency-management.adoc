...
| dvfs=3
| scale voltage up on demand for over-clocking (default). If `over_voltage` is specified in `config.txt` then the dynamic voltage scaling is disabled causing the system to revert to `dvfs=2`.
|===
