...
The network install feature uses HTTP over Ethernet to boot the Raspberry Pi into embedded xref:getting-started.adoc#imager-install[Raspberry Pi Imager].
...
To use HTTP boot, xref:../computers/configuration.adoc#update-bootloader-version[update] to a bootloader released 10th March 2022 or later. HTTP boot requires a wired Ethernet connection.

To use custom CA certificates, xref:../computers/configuration.adoc#update-bootloader-version[update] to a bootloader released 5th April 2024 or later. Only devices running the BCM2712 CPU support custom CA certificates.
...
* `<your_website>` with xref:raspberry-pi.adoc#HTTP_HOST[your website], for example, `yourserver.org`
* `<path_to_files>` with the xref:raspberry-pi.adoc#HTTP_PATH[path to your OS image] hosted on your website, for example, `path/to/files`
* `<hash>` with the hash value you generated above, for example, `701bd97f67b0f5483a9734e6e5cf72f9a123407b346088638f597878563193fc`
