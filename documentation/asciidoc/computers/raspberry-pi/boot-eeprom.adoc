...
NOTE: You can also reflash the bootloader using USB `rpiboot`. This method is typically used to mass provision Compute Modules. Reflashing the bootloader over USB is also the only option available for CM4 and CM4S. For more information, see xref:../computers/compute-module.adoc#attaching-the-touch-display-2-lcd-panel[Compute Module hardware].
