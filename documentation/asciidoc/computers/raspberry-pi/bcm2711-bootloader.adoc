...
The Raspberry Pi Imager application is configured with a json file downloaded at startup.

You can change the URL of the json file used by the Raspberry Pi Imager application to get it to offer your own images.
You can test this with the Windows or Linux versions of Raspberry Pi Imager by passing the URL via the `--repo` argument.
