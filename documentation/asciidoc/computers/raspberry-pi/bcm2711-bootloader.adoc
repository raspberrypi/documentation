...
On Raspberry Pi 5 `/WP` is pulled low by default and consequently write-protect is enabled as soon as the `Write Status Register` is configued. To clear write-protect pull `/WP` high by connecting `TP14` and `TP1`.
