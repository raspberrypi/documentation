...
[.whitepaper, title="Changing MAC addresses", subtitle="", link=https://pip.raspberrypi.com/categories/685-whitepapers-app-notes/documents/RP-003474-WP/Changing-MAC-addresses.pdf]
****
This whitepaper describes how to set a specific media access control (MAC) address on your Raspberry Pi device. It applies to both Ethernet and wireless interfaces.

Two mechanisms are described, one for devices up to the Pi 3 and one for the Pi 4/400 and CM 4. These latter three devices use the BCM2711 system on a chip (SoC), which has a built-in Ethernet port and hence defines its MAC addresses in a different way.
