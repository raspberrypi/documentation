...
[[needsoverclock,^**1**^]]
**1.** Only available with an overclocked core frequency: set `core_freq_min=600` and `core_freq=600`. See xref:config_txt.adoc#overclocking[overclocking].
...
In a system with multiple displays, using the legacy (pre-KMS) graphics driver, this forces a specific internal display device to be the first Linux framebuffer (`/dev/fb0`).
...
You can combine the rotation settings with the flips by adding them together. You can also have both horizontal and vertical flips in the same way. For example, a 180 degree rotation with a vertical and horizontal flip will be 0x20000 + 0x10000 + 2 = 0x30002.
