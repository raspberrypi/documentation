...
**Lite** variants of Compute Modules do not have on-board eMMC. Instead, follow the procedure to flash a storage device for other Raspberry Pi devices at xref:../computers/getting-started.adoc#install[Install an operating system].
...
TIP: In some cases, USB hubs can prevent the host device from recognising the Compute Module. If your host device doesn't recognise the Compute Module, try connecting the Compute Module directly to the host device. For more diagnostic tips, see https://github.com/raspberrypi/usbboot/blob/master/docs/troubleshooting.md[the usbboot troubleshooting guide].
...
+
. Download the latest Windows installer from the https://github.com/raspberrypi/usbboot/releases[releases] page or https://github.com/raspberrypi/usbboot[build `rpiboot` from source].
...
. Select **rpiboot - Mass Storage Gadget** from the start menu. After a few seconds, the Compute Module eMMC, NVMe or SD card appears as a USB mass storage device. This also provides a debug console as a serial port gadget.
...
You can use xref:../computers/getting-started.adoc#imager-install[Raspberry Pi Imager] to flash an operating system image to a Compute Module.
...
After the image has been written, disconnect and reconnect the Compute Module. You now see two partitions (for Raspberry Pi OS):
...
* The rpiboot version distributed with Ubuntu 24.04 and earlier has been known to fail to communicate with the device. To resolve this issue, https://github.com/raspberrypi/usbboot?tab=readme-ov-file#linux--cygwin--wsl[build `rpiboot` from source].
