...
NOTE: It is not possible to flash the eMMC on the Compute Module 1 from a Mac, due to a bug in its processor.

NOTE: There is a bug in the Compute Module 1 bootloader which returns a slightly incorrect USB packet to the host. A small number of USB host controllers object to this and cannot be used to flash the Compute Module 1.
