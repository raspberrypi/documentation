...
. Attach the silicone at the bottom of the cooler to the top of your CM5. Align the cut-out in the heat sink with the antenna https://en.wikipedia.org/wiki/Hirose_U.FL[U.FL connector] and the on-board antenna. Ensure that both the U.FL connector on the Compute Module, and the trapezoid shape area next to the U.FL connector (the on-board antenna), are visible through the cutout.
