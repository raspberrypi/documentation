...
NOTE: `dt-blob.bin` does not exist by default as there is a version compiled into `start.elf`, but for Compute Module projects it will often be necessary to provide a `dt-blob.bin` (which overrides the default built-in file).
+
...
NOTE: Please use the https://forums.raspberrypi.com/viewforum.php?f=107[Device Tree subforum] on the Raspberry Pi forums to ask Device Tree-related questions, or to post any issues, questions or bugs.
