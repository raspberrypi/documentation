...
* The second-stage boot loader is responsible for setting up the LPDDR2 SDRAM interface and other critical system functions. When set up, the second-stage boot loader loads and executes the main GPU firmware (`start.elf`).
...
https://pip.raspberrypi.com/documents/RP-008166-DS[`minimal-cm-dt-blob.dts`] is an example `.dts` device tree file. It sets up HDMI hot plug detection, an ACT LED, and sets all other GPIOs as inputs with default pulls.
...
Download https://pip.raspberrypi.com/documents/RP-008166-DS[`minimal-cm-dt-blob.dts`] and copy it to the boot partition in `/boot/firmware/`.
...
Download https://pip.raspberrypi.com/documents/RP-008165-DS[`example1-overlay.dts`], copy it to the boot partition in `/boot/firmware/`, then compile it with the following command:
...
After reboot, you see an `rtc0` entry in `/dev`. Run the following command to view the hardware clock time:
