...
If you wish to disable the touchscreen element and only use the display side, add the command `disable_touchscreen=1` to `/boot/firmware/config.txt` to do so.

To make the firmware to ignore the display even if connected, add `ignore_lcd=1` to `/boot/firmware/config.txt`.
