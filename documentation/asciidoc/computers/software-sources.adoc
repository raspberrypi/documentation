At Raspberry Pi we're trying to open source as much of our code as possible to make it easy to use and adapt for your own uses. Raspberry Pi develops and supports software specifically for our products and focuses its energy on software which provides the most optimal user experience. For example, drivers that optimise multimedia acceleration such as the 3D, HEVC decode, the camera imaging pipeline, AI acceleration, dual HDMI and audio.

== Finding software sources in Raspberry Pi OS

When looking for software which is distributed with Raspberry Pi OS, there are often upstream software sources that we patch to create our downstream packages (those tagged with `pass:[+rpt]`). To view the source for those packages it's usually easier to fetch through apt. To do this, you first need to edit your apt lists to include the source packages. Edit the following files to change each "Types: deb" line to "Types: deb deb-src":
...
The other place you can find our software in a more standard form is in our GitHub repositories; we have three GitHub organisations we use to publish our sources:

* https://github.com/raspberrypi
* https://github.com/raspberrypi-ui
* https://github.com/RPi-Distro

We also publish content related to our books and magazine in the following organisations:

* https://github.com/raspberrypipress
* https://github.com/themagpimag

== List of categorised sources
...
This repository contains the source code for the downstream Linux kernel supported and working on all versions of Raspberry Pi from 1 to 5.
...
A libcamera library implementation for controlling the Raspberry Pi device's ISP and cameras.
...
The taskbar or dock displayed on the screen when running the Wayland-based desktop.
...
The widgets displayed on the taskbars.
