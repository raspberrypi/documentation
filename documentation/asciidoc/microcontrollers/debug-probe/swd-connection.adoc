...
We recommend the use of the Raspberry Pi Pico VSCode extension, which integrates the use of OpenOCD and GDB, to upload and debug programs. See Chapter 4 of https://pip.raspberrypi.com/documents/RP-008276-DS[Getting started with Raspberry Pi Pico] for more information.

=== Standalone program upload

After you have built a binary:
...
[IMPORTANT]
======
To allow debugging, you must build your binaries as `Debug` rather than `Release` build type, for example,
