...
----
+
You see output similar to the following during the upgrade, indicating that Connect's services have migrated to the version 2 format:
+
[source,console]
----
...
If all is well, you see output similar to the following:
...
. You use a Wayland compositor such as wayfire or labwc, not X. You can control this setting through the *Advanced Options* in `raspi-config`.
. You use a desktop environment supported by WayVNC, for example, Raspberry Pi Desktop. For instance, using KDE switches your Wayland compositor to kwin, which is unsupported.
. You have an active graphical desktop session running as the same user as the one you signed into. For most, this means enabling *Desktop Autologin* in the *System Options* in `raspi-config`.
...
For screen sharing, Connect can only share an existing graphical desktop session: it does not create entirely new sessions. There must already be a desktop session in progress. To start such a session automatically on boot, enable *Desktop Autologin* in the *System Options* in `raspi-config`.
...
If Connect can communicate properly on your network, you see output similar to the following:
...
If the A/B boot configuration or remote update process fails, refer to the following list of common faults and fixes.
...
a|The recipient device must be able to locate the update file. Open a remote shell connection to the recipient device, then use curl to check whether the device can access the IP address contained in the URI you provided when creating the deployment.
...
|Make sure that the URI you provided when creating the deployment contains the URL of the `.tar.zst` update file, not the `.img` full image file.
...
Teh command displays output similar to the following:
...
Add the environment variable `RPI_CONNECT_VERBOSE=1` to `/etc/default/rpi-connect`, and restart Connect with the following command:
...
Alternatively, for versions prior to 2.7 override the `rpi-connect` service definition with the following command:
