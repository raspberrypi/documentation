...
Raspberry Pi Imager then uses the `.img` file to write the operating system configuration to the storage device. This includes opting the device in to OTA (over-the-air) updates so that Raspberry Pi Connect can update it remotely.
