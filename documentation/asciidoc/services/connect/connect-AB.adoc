...
Configuring for A/B boot requires being able to physically access the device. This is because you must image the device with the appropriate A/B boot layout. However, once configured, you can then deliver remote updates using Raspberry Pi Connect Remote Update.
...
The following are required to use Raspberry Pi Connect Remote Update and the A/B boot configuration:
...
+
The service supports HTTP, HTTPS, FTP, SFTP, and File URI.
+
...
** Opted in to OTA updates.
** Registered with, and signed in to, your Raspberry Pi Connect account.

==== Implement A/B boot and remotely update the first image

Follow the four stages below to implement the A/B configuration and send updates using Raspberry Pi Connect Remote Update:
...
. Stage 4: Use the Raspberry Pi Connect Remote Update feature to select the recipient device, then deploy the update artefact to that device.
...
Raspberry Pi Imager then uses the `.img` file to write the operating system configuration to the storage device. This includes opting the device in to OTA (over-the-air) updates so that Raspberry Pi Connect Remote Update works.
...
TIP: rpi-image-gen contains an example application, with layers and configuration, to build an image compatible with Raspberry Pi Connect Remote Update.
...
The device is now ready to update using Raspberry Pi Connect Remote Update.
...
Now the device is configured for A/B booting, deliver update artefacts to it using Raspberry Pi Connect Remote Update.

Update artefacts are created by rpi-image-gen as `.tar.zst` files. These are much smaller than the `.img` file you created in Stage 1, and are deployed in Stage 4 to the recipient device by Raspberry Pi Connect Remote Update.
...
Now that you’ve created an update artefact to deploy using Raspberry Pi Connect Remote Update, you can host it on an FTP/SFTP server, save it locally using a `file://` URI, put it on a public web server, an S3 bucket, or any other website that the recipient device can download from using HTTP.

Next, you register the URI and SHA-256 checksum with Raspberry Pi Connect. This is so that the remote update service can tell the device – which you select in Stage 4 – where to find the update file.
...
If the A/B boot configuration or Connect Remote Update process fails, refer to the following list of common faults and fixes.

You can also refer to the https://github.com/raspberrypi/rpi-image-gen/blob/master/docs/execution/index.adoc[rpi-image-gen documentation] and visit the https://forums.raspberrypi.com/viewforum.php?f=28&sid=376757fde13953b7bfdcc0b1061edeca[Raspberry Pi Troubleshooting forum].

[TIP]
====
If you see an error, use Raspberry Pi Connect's remote shell access to view the `systemd` journal for additional information:
...
a|`Download could not connect to server (retry).`
a|The recipient device must be able to locate the update file. Open a remote shell connection to the recipient device, then use curl to check whether the device can access the IP address contained in the URI you provided in Stage 3, Step 4.

[source,console]
----
$ curl -I http://192.168.0.1:8080
----

a|`update file rejected.`
|Make sure that the URI provided in Stage 3, Step 4 contains the URL of the `.tar.zst` update file, not the `.img` full image file.

a|No *Deploy* option appears next to the device in Raspberry Pi Connect.
a|Enable OTA opt-in using the following command, then refresh your browser:
