...
Raspberry Pi Connect, along with its API and software components, has undergone security assessments by penetration testing and security analysts at https://cure53.de[Cure53]. You can read the https://cure53.de/pentest-report_raspberry-web.pdf[full report] on the Cure53 site.

For more details on WebRTC security, see https://webrtc-security.github.io/[A Study of WebRTC Security].

For xref:../services/connect-for-organisations.adoc[**Connect for Organisations**] customers, we provide an xref:../services/connect-for-organisations.adoc#audit-log[audit log] to review activity from the past 90 days. To preserve privacy, we store only the country code when geolocating events by IP address.
