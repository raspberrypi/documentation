...
+
Alternatively, you can create an auth key by selecting **Settings** from a personal or organisation's Connect account, and provide this to the command to sign in automatically:
+
[source,console]
----
...
----
+
Connect automatically detects an auth key saved to `+.config/com.raspberrypi.connect/auth.key+` in your home directory and attempts to sign in with it.
