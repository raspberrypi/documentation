...
Click the Connect system tray icon to open the Connect menu. This menu shows your current sign in status, an option to sign out, and an option to enable or disable screen sharing. You can also use the terminal command `rpi-connect signout` to remove your Raspberry Pi from Connect.
...
Now that your Raspberry Pi is listed in the Connect web portal, you can access your Raspberry Pi from anywhere using only a browser. Connect provides multiple ways to interact with your Raspberry Pi remotely.

==== Screen share from your Raspberry Pi
...
image::images/connected.png[width="80%"]
...
In the Connect portal, the **Screen sharing** badge and the **Screen sharing** option in the **Connect via** menu will appear crossed-out.
...
==== Remotely access a terminal shell running on your Raspberry Pi

Connect includes the ability to access a terminal shell running on your Raspberry Pi from a browser. Use the following instructions to access the remote shell.
...
Click the **Connect via** button to the right of the device you would like to access. Select the **Remote shell** option from the menu. This opens a terminal session your Raspberry Pi.
...
Once connected, a green dot appears next to the **Remote shell** badge in the Connect portal. This indicates an active remote shell session. Hover to see the current number of screen sharing sessions.
...
To turn off screen sharing, click the Connect system tray icon and unselect **Allow remote shell**. Your Raspberry Pi remains signed into Connect, but you won't be able to create a remote shell session from the Connect dashboard.
...
In the Connect portal, the **Remote shell** badge and the **Remote shell** option in the **Connect via** menu will appear crossed-out.
...
=== Manage your Raspberry Pi devices

Connect lists all of the Raspberry Pi devices linked with your Raspberry Pi ID. Each device has a **Connect** button to open a screen sharing session, if screen sharing is enabled on that device. Each device has a list of badges under the device name to indicate available services.
