...
* Somewhere to host the artefact from which the recipient device can download it. For the hosting options that Connect supports, see <<host-the-update-artefact,Host the update artefact>>.
...
. Stage 3: Host the update artefact so that the recipient device can download it.
. Stage 4: Use Raspberry Pi Connect's remote update feature to deploy the update artefact to the recipient device, then verify the update.
...
Now that you’ve created an update artefact to deploy using Raspberry Pi Connect, host it somewhere that the recipient device can download it from. For the hosting options Connect supports, and the details you need to collect, see <<host-the-update-artefact,Host the update artefact>>.
...
===== Step 1: Deploy the artefact to the recipient device

Deploy the artefact from the https://connect.raspberrypi.com/[Raspberry Pi Connect website], using the URI and checksum you copied down in Stage 3, as described in <<update-remotely,Deploy the update artefact>>.

===== Step 2: Verify the update

When the deployment shows as *Succeeded*, connect to the recipient device using Raspberry Pi Connect and confirm the deployed version, as in Stage 1, Step 7:
