...
This can be done by using the [raspi-config](../raspi-config) utility, or manually.
```
sudo raspi-config
```
...
For full instructions on how to use Device Tree Overlays see [this page](../device-tree). In brief, add a line to the `config.txt` file to enable Device Tree Overlays. Note that the `-overlay.dts` part of the filename is removed.
```
...
