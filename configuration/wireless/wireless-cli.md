...
If you are using the `wpa_passphrase`–encrypted PSK, you can either copy and paste the encrypted PSK into the `wpa_supplicant.conf` file, or redirect the tools output to your configuration file by calling `wpa_passphrase "testing" >> /etc/wpa_supplicant/wpa_supplicant.conf` and then enter the password when asked. To do the latter, you will need the necessary administrative privileges. To get them, either change to `root` by executing `sudo su`, or you can use `wpa_passphrase "testing" | sudo tee -a /etc/wpa_supplicant/wpa_supplicant.conf > /dev/null` and give it the testing password when asked; this will append the passphrase without you having to change to `root`. Lastly, **make sure you use `>>`, or use `-a` with `tee`** — either will append text to an existing file. Using a single chevron `>`, or omitting `-a` when using `tee`, will erase all contents and **then** append the output to the specified file. Note that the redirection to `/dev/null` at the end of the second form simply prevents `tee` from **also** outputting to the screen (standard output).
