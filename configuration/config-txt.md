...
For other modes not listed here see [this thread](http://www.raspberrypi.org/phpBB3/viewtopic.php?f=29&t=24679).
...
Specifies the console framebuffer height in pixels. The default is the display height minus the overscan.
...
If you've got multiple Raspberry Pis, and you've bought a codec licence for each of them, you can list up to 8 licence keys in a single `config.txt`. This enables you to swap the same SD card between the different Raspberry Pis, for example `decode_MPG2=0x12345678,0xabcdabcd,0x87654321`.
...
This enables dynamic clocks and voltage for the ARM core, GPU core and SDRAM. When busy, the ARM frequency goes up to `arm_freq` and down to `arm_freq_min` on idle.
...
To monitor the Pi's PSU voltage, you'll need a multimeter to measure between the TP1 and TP2 power supply test points; more information is available [here](../troubleshooting/power.md).
