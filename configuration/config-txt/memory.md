...
GPU memory in megabytes, sets the memory split between the CPU and GPU; the CPU gets the remaining memory. The minimum value is `16`; the maximum value is `192`, `448`, or `944`, depending on whether you are using a 256M, 512MB, or 1024MB Pi. The default value is `64`. For Raspberry Pi 4 devices with more than 1GB RAM, the defaults for the 1024MB option apply.
...
The `gpu_mem_1024` command sets the GPU memory in megabytes for the 1024MB (or more) Raspberry Pi devices (it is ignored if memory size is smaller than 1024MB). This overrides `gpu_mem`. The maximum value is `944`, and the default is not set.
