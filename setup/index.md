...
- [SD Card](./../installation/sd-cards)
    - We recommend an 8GB class 4 SD card, ideally preinstalled with [NOOBS](./../installation/noobs).
...
- Keyboard and mouse
    ...
    - For keyboard layout configuration options see [raspi-config](./../configuration/raspi-config).
...
- Audio lead
    ...
    - No separate audio lead is necessary if you're using an HDMI cable to connect to a monitor with speakers, as audio can be played directly through the display; but it is possible to connect one if you prefer to have the audio played through other speakers - this requires [configuration](./../configuration/audio-config).
