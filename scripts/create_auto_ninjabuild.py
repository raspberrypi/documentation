...
if __name__ == "__main__":
    ...
    with open(output_ninjabuild, 'w') as fh:
        ...
        for page in sorted(doc_pages):
            for include in sorted(join_files[page]):
                ...
                if source not in all_doc_sources:
                    ...
                    extra_sources = ['$scripts_dir/create_build_adoc_include.py', '$scripts_dir/create_build_adoc.py', '$site_config', '$GITHUB_EDIT_TEMPLATE']
        ...
        for page in sorted(static_pages):
            ...
            dest = os.path.join('$out_dir', page)
            source = os.path.join('$src_dir', page)
            extra_sources = ['$scripts_dir/create_build_adoc_doxygen.py', '$scripts_dir/create_build_adoc.py', '$documentation_index', '$site_config', '$PAGE_PREAMBLE_TEMPLATE', '$DOXYGEN_PICOSDK_INDEX_JSON']
            if source not in all_doc_sources:
                all_doc_sources.append(source)
